...
// Config contains the configuration required when creating new page-rank
...
func (c *Config) validate() error {
	...
	if c.DampingFactor < 0 || c.DampingFactor > 1.0 {
		err = multierror.Append(
			err, errors.New("DampingFactor must be in the range of (0, 1]"),
		)
	...
	if c.MinSADForConvergence < 0 || c.MinSADForConvergence >= 1.0 {
		err = multierror.Append(
			err, errors.New("MinSADForConvergence must be in the range (0, 1)"),
		)
