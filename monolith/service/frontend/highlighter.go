...
type matchHighlighter struct {
	// A regex for matching search terms as standalone words at the beginning,
	// middle or end of a sentence.
	regexes []*regexp.Regexp
}

// newMatchHighlighter returns a matchHighlighter instance that can highlight
// specified searchTerms in blocks of text / sentences.
func newMatchHighlighter(searchTerms string) *matchHighlighter {
	...
	for _, token := range strings.Fields(strings.Trim(searchTerms, `"`)) {
		re, err := regexp.Compile(fmt.Sprintf(`(?i)%s`, regexp.QuoteMeta(token)))
		if err != nil {
			continue
		}
